                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                <option value="number" {{ old('type', $setting->type) == 'number' ? 'selected' : '' }}>{{ __('Number') }}</option>
                                <option value="file" {{ old('type', $setting->type) == 'file' ? 'selected' : '' }}>{{ __('File') }}</option>
                                <option value="select" {{ old('type', $setting->type) == 'select' ? 'selected' : '' }}>{{ __('Select') }}</option>
                                <option value="textarea" {{ old('type', $setting->type) == 'textarea' ? 'selected' : '' }}>{{ __('Textarea') }}</option>
                                <option value="code" {{ old('type', $setting->type) == 'code' ? 'selected' : '' }}>{{ __('Code') }}</option>
                                <option value="radio" {{ old('type', $setting->type) == 'radio' ? 'selected' : '' }}>{{ __('Radio') }}</option>
                                <option value="checkbox" {{ old('type', $setting->type) == 'checkbox' ? 'selected' : '' }}>{{ __('Checkbox') }}</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="active" {{ old('status', $setting->status) === 'active' ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        let codeEditor = null;
        const existingValue = {!! json_encode($setting->value) !!};
        ...
        $('#type').on('change', function() {
            ...
            switch(type) {
                case 'text':
                    container.html(`<input type="text" class="form-control" name="value" value="{{ old('value', $setting->value) }}">`);
                    break;
                case 'number':
                    container.html(`<input type="number" class="form-control" name="value" value="{{ old('value', $setting->value) }}">`);
                    break;
                case 'file':
                    container.html(`
                        ...
                        @if($setting->value)
                            <div class="mt-2">
                                <small>Current file: <a href="{{ asset('assets/'.$setting->value) }}" target="_blank">{{ $setting->value }}</a></small>
                            </div>
                        @endif
                    `);
                    break;
                case 'select':
                    let selectOptions = '';
                    if (existingValue && existingValue.options) {
                        existingValue.options.forEach((option, index) => {
                            selectOptions += `
                                <div class="option-item mb-2">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="value[options][]" value="${option}" placeholder="Option value">
                                        <input type="text" class="form-control" name="value[labels][]" value="${existingValue.labels[index]}" placeholder="Option label">
                    ...
                    break;
                case 'textarea':
                    container.html(`<textarea class="form-control" name="value" rows="3">{{ old('value', $setting->value) }}</textarea>`);
                    break;
                case 'code':
                    container.html(`<textarea class="form-control" name="value" id="valueEditor">{{ old('value', $setting->value) }}</textarea>`);
